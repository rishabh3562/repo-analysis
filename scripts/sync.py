...
def generate_sync_report(repo_events):
    ...
    active = [e for e in repo_events if e["count"] > 0]
    md += f"**Repos synced:** {len(repo_events)}\n"
    md += f"**Total new events:** {total_events}\n\n"

    md += "## Per-Repo Breakdown\n\n"
    if active:
        for e in sorted(active, key=lambda e: e["count"], reverse=True):
            md += f"### {e['repo']}\n"
            md += f"- **Events:** {e['count']}\n"
        ...
        md += "No repos had new events this run.\n\n"

    quiet = len(repo_events) - len(active)
    if quiet:
        md += f"*{quiet} other repos synced with no new events.*\n\n"
